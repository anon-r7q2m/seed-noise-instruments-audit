#!/usr/bin/env python3
"""App E PolyPythias: ten-seed ground truth, 2 panels.
  ...
  (a) per-seed final deviations from the cell mean, in percentage points --
      ...
      divergent run (blimp 31m, -3.9 pp) carries a direct label.
  (b) proxy vs 10-seed truth ratio, 9 cells; parity and sqrt(2) references.
      ...
      (ratio 0.55; the 9-cell median moves 1.8x -> 1.3x).
Task identity lives in the title chip row; axes carry data only.
Data: data/analysis/pp_tidy.parquet, data/tables/pp_final_seed_noise.csv,
data/tables/pp_t1_ratio.csv. Paper numbers asserted in-script.
...
R_DIR = os.environ.get("NFT_R", os.path.join(_ROOT, "data", "analysis"))
T_DIR = os.environ.get("NFT_TABLES", os.path.join(_ROOT, "data", "tables"))
...
# beeswarm (deterministic, symmetric, non-overlapping) instead of random jitter:
# show every seed, packed so no two points overlap.
