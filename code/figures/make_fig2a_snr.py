...
Two series, recomputed from data/analysis/dd_tidy.parquet with the exact
audit() of verify-scripts/rule3_repro.py:
  observed     = sd(recipe means) / mean within-recipe sd       (measurement)
  deconvolved  = sqrt(max(var(means) - mean(sd^2)/3, 0)) / ...  (matches
                 Table 1's SNR column; asserted below)
SNR = 1 threshold dashed; the <=60M noise-dominated side marked at the
60M-90M log-midpoint; the 60M->90M jump annotated; in-place legend.

Output: out/fig2a_snr_cliff.pdf (1.78 x 1.40 in, Fig-2-row size).
"""
import os
import numpy as np
import pandas as pd
...
# ------------------------------------------------------------- data (canonical)
R_LAKE = os.environ.get("NFT_R", os.path.join(_ROOT, "data", "analysis"))
