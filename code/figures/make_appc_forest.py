#!/usr/bin/env python3
"""App C forest plot: source equivalence, task by task.

Per-task sigma_init/sigma_order with F-interval CIs vs the [0.5, 2] band.
Single column of 27 rows (grouped: 9 primary, gap, 18 bpb). rliable idiom:
CI = horizontal bar, point estimate = black tick. The two tasks whose CI
excludes 1 (hellaswag below, winogrande above) are highlighted in vermillion.
Data: data/tables/t2_source_equivalence.csv; the 0/27 + 2-excluders claims
are asserted in-script.
