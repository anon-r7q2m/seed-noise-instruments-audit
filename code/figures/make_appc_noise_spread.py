#!/usr/bin/env python3
"""App C noise x spread plane: per-task noise vs spread on the S&N 1B arms.

Per-task scatter of noise (sigma_init, SD across seeds) vs spread
(sigma_order, SD across recipes), log-log, square decades so parity sits at a
true 45 degrees. Circles = primary readout, crosses = bpb readout; grey
pair-lines join the 8 task identities measured under both. Dashed diagonal =
parity; dotted diagonals = the [0.5, 2] equivalence band. Only the two
excluders (hellaswag, winogrande) carry their F tick (vermillion,
anti-diagonal = the ratio coordinate). Dense minerva cluster fans out to the
right with hairline leaders.

Data: data/tables/t2_source_equivalence.csv (the forest's own table).
...
CSV = os.environ.get("NFT_T2", os.path.join(_ROOT, "data", "tables", "t2_source_equivalence.csv"))
