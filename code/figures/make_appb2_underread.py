...
Per scale, median over task-recipe cells of (3-seed truth SD / proxy
step-noise), with the sqrt(2) two-source-bundling reference.
Data: data/tables/t1b_summary_by_size.csv (med_ratio column) and
data/analysis/t1b_cells.parquet; the ~1.6x at 1B claim is asserted in-script.
...
CSV = os.path.join(os.environ.get("NFT_TABLES", os.path.join(_ROOT, "data", "tables")), "t1b_summary_by_size.csv")
...
CELLS = os.path.join(os.environ.get("NFT_R", os.path.join(_ROOT, "data", "analysis")), "t1b_cells.parquet")
...
# gate: the paper's headline "~1.6x at 1B"; the 750M dip below 1 is data, not a bug
