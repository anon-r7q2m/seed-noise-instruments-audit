...
Rank-rank scatter (1 = best), 25 recipes, per readout:
  (b) accuracy macro avg: 4M rank vs 1B rank -- inverts (Pearson r = -0.56 on
      scores, Spearman rho = -0.52 on ranks; headline numbers asserted below).
...
# ---- rcParams baseline (final physical size, no post-scaling) ----
plt.rcParams.update({
    "font.family": FONT,
    "font.size": 7.0,
    "axes.linewidth": 0.6,
...
def render(df, color, title, ann_main, ann_sub, fname, stats_corner):
    ...
    # ann_sub: Spearman rho on the plotted ranks (kept visible on rank axes)
    if ann_sub:
