...
Reads tmp/data/sn_random_seeds.parquet (10 init-seed runs, 9 data-order runs, 1B-5xC) and
data/analysis/dd_tidy.parquet; writes data/tables/t2_source_equivalence.csv and
data/tables/t2_dd1b_vs_sn_bundle.csv.
Estimators: score = mean over last 3 ckpts <= step 69000; sigma_init/sigma_order = relSD
across arm runs; ratio CI via F-quantile (df 9/8) and 2000-draw run bootstrap;
share_init = s_i^2/(s_i^2+s_o^2); bundled prediction sqrt(s_i^2+s_o^2).
"""
