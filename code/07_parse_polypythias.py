#!/usr/bin/env python3
"""Parse PolyPythias lm-eval JSONs into a tidy table.

Input : tmp/pp_json/pythia-{size}-seed{k}/step{S}/{model_dir}/results_{timestamp}.json
Dedupe: for (run,step,task,metric) keep the latest timestamp.
Output: data/analysis/pp_tidy.parquet (size, seed, step, task, metric, value, ts)
"""
