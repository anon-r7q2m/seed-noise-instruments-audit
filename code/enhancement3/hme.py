...
EB moment fits, grid posteriors for per-recipe noise, MC posterior integration
for pair estimands (p_flip, p_err, excess risk), recipe-/task-cluster
bootstrap, parametric-bootstrap validation. Pure numpy/pandas/scipy, no MCMC.
"""
import numpy as np
...
def cell_summary(fit, M, rng, keep_pairs=False):
    ...
    out = dict(
        ...
        # noise-admitting share of error mass (model-based)
