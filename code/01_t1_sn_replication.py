#!/usr/bin/env python3
"""T1 part A — replicate the S&N final-n-checkpoint proxy claim on the released 1B random_seeds split.

Input : tmp/data/sn_random_seeds.parquet (10 init-seed runs, 9 data-order runs, 1 high-eval run; 21 tasks)
Outputs: data/tables/t1a_sn_replication_per_task.csv, data/tables/t1a_sn_replication_summary.csv
Estimators: x_b = mean over seed-arm runs of relSD(final 30 ckpts, 500-step grid);
            y_init/y_data = relSD across runs of final score (mean of last 3 ckpts <= step 69000).
"""
