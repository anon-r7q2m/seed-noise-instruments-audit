#!/usr/bin/env python3
"""Head-to-head of the deconvolved separability SNR vs the checkpoint-spread-proxy
SNR as predictors of per-task pairwise decision accuracy vs the 1B target, same
data, same cells:
...
Spearman across all (task, scale) cells, plus per-scale medians.
Writes sn_headtohead.json and sn_headtohead_cells.csv. NFT_R overridable.
"""
import os, json
import numpy as np
import pandas as pd
...
df.to_csv(os.path.join(HERE, "sn_headtohead_cells.csv"), index=False)
with open(os.path.join(HERE, "sn_headtohead.json"), "w") as f:
    json.dump(OUT, f, indent=1)
print("wrote sn_headtohead.json + cells csv")
