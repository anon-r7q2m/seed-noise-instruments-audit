#!/usr/bin/env python3
"""crossed_grid: crossed-design main analysis.

Design: 16 cells (4 init x 4 order) x 3 full replicates = 48 runs
(21 earlier + 27 later runs). Y_idr = per-task final-step accuracy.
...
- Sensitivity B (batch): same estimators with an earlier/later-batch fixed effect
  added (same OLS/moment estimator both ways); if the dominance verdict
  changes, report batch-sensitive.
- Verification chain: NaN scan (done separately); earlier-batch-only recompute must
  match the archived grid20m_analysis.json grid values within 1e-6 relative.

Writes crossed_grid.json. Pure CPU, deterministic.
"""
import os, json
import numpy as np
import pandas as pd
...
RUNS = os.environ.get("NFT_STAGE2_RUNS",
    os.path.join(_ROOT, "data", "self_run_controls"))
ARCH = os.environ.get("NFT_STAGE2_JSON",
    os.path.join(_ROOT, "data", "self_run_controls", "grid20m_analysis.json"))

TASKS = ["blimp", "lambada_openai", "social_iqa_local", "arc_easy", "arc_challenge", "piqa_local"]

# run dirs: grid iX_oY; reps repN_iX_oY
REP2 = {"i1_o1": "rep1_i1_o1", "i2_o2": "rep2_i2_o2", "i3_o3": "rep3_i3_o3",
        "i4_o4": "rep4_i4_o4", "i2_o3": "rep5_i2_o3"}
# later-batch runs iX_oY_rK: rep cells got r1; others got r1, r2
STAGE3 = {}
for i in range(1, 5):
    for j in range(1, 5):
        c = f"i{i}_o{j}"
        STAGE3[c] = ["%s_r1" % c] if c in REP2 else ["%s_r1" % c, "%s_r2" % c]
...
BUNDLE = os.environ.get("NFT_STAGE3_BUNDLE",
    os.path.join(_ROOT, "data", "self_run_controls", "grid20m_full_evals.json"))
_bundle_cache = {}
...
def build_frame():
    ...
    for i in range(1, 5):
        for j in range(1, 5):
            cell = f"i{i}_o{j}"
            dirs = [("batch1", f"i{i}_o{j}")]
            if cell in REP2:
                dirs.append(("batch1", REP2[cell]))
            for d3 in STAGE3[cell]:
                dirs.append(("batch2", d3))
...
def main():
    ...
    json.dump(out, open(os.path.join(HERE, "crossed_grid.json"), "w"), indent=1)
    print("wrote crossed_grid.json")
