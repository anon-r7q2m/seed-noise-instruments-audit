#!/usr/bin/env python3
"""sn_exact_protocol: replicate S&N's Fig-7 result under their *exact* protocol,
...
Writes sn_exact_protocol.json. Pure CPU, deterministic. NFT_SN overridable.
"""
import os, json
import numpy as np
import pandas as pd
...
if not SN or not os.path.exists(SN):
    for cand in [os.path.join(_ROOT, "data", "analysis", "random_seeds.parquet")]:
...
#  openbookqa/piqa/socialiqa/winogrande; copycolors is bpb-only there)
...
out = {"published_fig7": {"init_R2": 0.82, "order_R2": 0.86},
       ...
       "rows": rows}
json.dump(out, open(os.path.join(HERE, "sn_exact_protocol.json"), "w"), indent=1)
print("wrote sn_exact_protocol.json")
