# Acceptance gate: every number in abstract/intro/S5/S7 checked against
...
SN = os.environ.get("NFT_SN", os.path.join(_ROOT, "data", "analysis", "random_seeds.parquet"))
TEX = os.environ.get("NFT_TEX", os.path.join(_ROOT, "tex_snapshot"))
...
def _read_tex(p):
    # strip comment lines so that numbers mentioned in comments do not trip the gate
...
# 0/27 in abstract, MC calibration numbers in S3/appB, jump LORO in S5
t2 = pd.read_csv(os.environ.get("NFT_T2", os.path.join(_ROOT, "data", "tables", "t2_source_equivalence.csv")))
...
# rule 4 multiples: band expressed in units of the LARGER side's SD; the range over
# the full imbalance sweep is 2.05-2.48, endpoints 2.27/2.48
