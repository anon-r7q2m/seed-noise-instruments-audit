#!/usr/bin/env python3
"""Ablation of chance-indistinguishable tasks from the 4M macro ranking:
drop tasks whose best 4M recipe is within Welch noise of chance (V1) or whose
per-task 4M pairwise-accuracy bootstrap CI covers 50% (V2), then recompute the
4M-1B macro correlation (Pearson/Spearman + family-cluster bootstrap CI) on the
kept tasks; writes chance_task_ablation.json.
"""
import os, json
import numpy as np
import pandas as pd
...
# recipe families: the 10-family mapping used by the family-cluster bootstrap
FAM = {}
FAM["C4"] = "c4"
...
# sanity gate: reproduce the family-bootstrap headline on all 10 tasks
...
with open(os.path.join(HERE, "chance_task_ablation.json"), "w") as f:
    json.dump(OUT, f, indent=1)
print("wrote chance_task_ablation.json")
