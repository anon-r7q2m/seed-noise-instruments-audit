#!/usr/bin/env python3
"""Canonical generators for the intermediate JSONs (rewrites them in place):
null-band share under pure sampling noise, per-scale SNR-vs-accuracy Spearman,
seed-unstable share among errors, top-k regret vs the 1B target, family-cluster
bootstrap of the 4M-1B macro correlation, BH decidable share + SNR->accuracy
bins, broken-pairing q3 null, and two-way-ANOVA seed variance share.
Prints PASS/FAIL per item and writes regen_report.json.
"""
import os, json, sys
import numpy as np
import pandas as pd
...
sys.path.insert(0, os.environ.get("NFT_HME_SRC", os.path.join(_ROOT, "code", "enhancement3")))
import hme
...
json.dump({"null_band_m5_n3": share, "definition": "P(0.5 <= sqrt(chi2_2/2)/sqrt(chi2_4/4) <= 2) by quadrature"},
          open(os.path.join(HERE, "null_band.json"), "w"), indent=1)
rec("null_band", abs(share - 0.6794) < 0.005, round(share, 4), "0.6794 (paper prints 67.9%)")
...
json.dump(oos, open(os.path.join(HERE, "oos_snr.json"), "w"), indent=1)
rec("oos_snr", abs(oos["150M"]-0.903)<0.005 and abs(oos["530M"]-0.915)<0.005,
...
json.dump(excl, open(os.path.join(HERE, "excl750.json"), "w"), indent=1)
rec("excl750", abs(excl["macro_excl750"]-0.5864)<0.005 and abs(excl["tasklevel_excl750"]-0.8139)<0.005,
...
json.dump(tk, open(os.path.join(HERE, "topk_regret.json"), "w"), indent=1)
rec("topk_regret", abs(tk["4M"]["top1"]-0.0595)<1e-3 and abs(tk["150M"]["top1"]-0.0466)<1e-3
...
# merge: preserve the argmax / blocks_1b / acc4 sections produced by claim_verification.py
_prev = json.load(open(os.path.join(HERE, "family_bootstrap.json"))) if os.path.exists(os.path.join(HERE, "family_bootstrap.json")) else {}
for k in ("argmax", "blocks_1b", "acc4"):
    if k in _prev: fb[k] = _prev[k]
json.dump(fb, open(os.path.join(HERE, "family_bootstrap.json"), "w"), indent=1)
rec("family_bootstrap", abs(fb["family_boot"]["lo"]+0.72)<0.02 and abs(fb["family_boot"]["hi"]+0.08)<0.02,
...
json.dump(p3, open(os.path.join(HERE, "prescription3.json"), "w"), indent=1)
...
rec("prescription3 BH shares", bh_ok, {k: round(v,3) for k,v in bh.items()},
    "0/0/0/0/.37/.51/.55/.43/.62")
bvals = [got_bins[k] for k in got_bins]
rec("prescription3 bins (revised canonical)",
...
json.dump(q3, open(os.path.join(HERE, "q3_ci.json"), "w"), indent=1)
rec("q3_ci", abs(q3["median"]) < 0.005 and abs(q3["frac_negative"]-0.5) < 0.05,
...
json.dump(ss, open(os.path.join(HERE, "seed_share.json"), "w"), indent=1)
rngpct = [v for k, v in ss.items() if k != "definition"]
rec("seed_share", max(rngpct) <= 0.056 and min(rngpct) >= 0.005,
    {k: round(v,4) for k,v in ss.items() if k != "definition"}, "paper prints range 0.6-5.5%")

with open(os.path.join(HERE, "regen_report.json"), "w") as f:
