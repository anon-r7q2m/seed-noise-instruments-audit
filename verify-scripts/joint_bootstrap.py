#!/usr/bin/env python3
"""Dependence-matched uncertainty for the two headline ratios:
(a) 4M macro-1B ranking correlation under a joint bootstrap over recipe
    families and the 3 seeds within each picked recipe (B=2000);
(b) pooled median y/x magnitude ratio under a family x task bootstrap with
    chi2_2 seed-noise propagation within each cell (B=2000);
writes joint_bootstrap.json.
"""
import os, json
import numpy as np
import pandas as pd
...
CACHE = os.path.join(HERE, "cell_seeds_cache.pkl")
...
with open(os.path.join(HERE, "joint_bootstrap.json"), "w") as f:
    json.dump(OUT, f, indent=1)
print("wrote joint_bootstrap.json")
