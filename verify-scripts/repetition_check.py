#!/usr/bin/env python3
"""Is the 4M-vs-1B ranking inversion explained by data repetition at 1B? Small
filtered pools repeat at 1B (100 tokens/param) but not at 4M; recompute the 4M
ranking's correlation with the 1B ranking on all 25 recipes vs only the
non-repeating ones (family-cluster bootstrap CI). Pool sizes are in tokens
(shard bytes/2, uint16 memmaps); repetition factor at scale S =
100*S_tokens / pool_tokens, >1 means the run wraps the pool.
Writes repetition_check.json. NFT_R overridable.
"""
import os, json
import numpy as np
import pandas as pd
...
# Pool sizes in billions of tokens.
POOL_BT = {
 "C4": 138.4,
...
with open(os.path.join(HERE, "repetition_check.json"), "w") as f:
    json.dump(OUT, f, indent=1)
print("wrote repetition_check.json")
