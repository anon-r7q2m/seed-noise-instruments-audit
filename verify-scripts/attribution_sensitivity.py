#!/usr/bin/env python3
"""Sensitivity of the 150M seed-noise attribution share (noise_share =
exp_errors_noise / exp_errors) to the noise likelihood, rank transform,
partial-pooling strength, leave-one-recipe-out, and target-side noise df;
writes attribution_sensitivity.json.
"""
import sys, json
import numpy as np
import pandas as pd
...
DD = os.environ.get("NFT_DD", os.path.join(_ROOT, "data", "analysis", "dd_tidy.parquet"))
...
# ------------------------------------------------------------- dump
with open("attribution_sensitivity.json", "w") as f:
    json.dump(out, f, indent=1)
print("\nwrote attribution_sensitivity.json")
