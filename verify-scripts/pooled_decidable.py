#!/usr/bin/env python3
"""Pooled-variance decidable shares and related checks:

(a) decidable share under a POOLED variance (at het ratio ~1.1-1.2 the pair-specific
    Welch df 2-4 is the lowest-power legal choice; pooled df~50 is defensible and
    halves the bands). Replicates the pair-specific BH shares first (must match
    prescription3.json), then reports the pooled-variant shares under BH and BY.
(b) full per-scale macro-vs-1B Pearson series to check the monotonicity of
    "correlation climbs monotonically to +0.91 at 530M" (6M--14M values included).
(c) k(R0) at R0 in {0.80, 0.85, 0.90} (80% power, alpha=0.05, the panel's
    m=5/n=3/sigma_a=0.74 operating point).
(d) Table 2 prints median noise but the SNR denominator is RMS noise -- emit the
    RMS column values for the table.

Pure CPU, deterministic. NFT_R overridable. Writes pooled_decidable.json.
"""
import os, json
import numpy as np
import pandas as pd
...
rep = json.load(open(f"{HERE}/prescription3.json"))
out_a, ok_rep = {}, True
for sz in S9:
    sc = final_scores(sz)
...
json.dump({"pooled_shares": out_a, "welch_replication_ok": bool(ok_rep),
          ...
          open(os.path.join(HERE, "pooled_decidable.json"), "w"), indent=1)
print("wrote pooled_decidable.json")
