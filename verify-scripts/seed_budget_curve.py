#!/usr/bin/env python3
"""Decidable share (BH q=0.05) vs seed budget n, per scale. With n seeds the Welch
band half-width for pair (i,j) scales as t(0.975, nu)*sqrt((s_i^2+s_j^2)/n); holding
the measured 3-seed SDs fixed, the share is computable in closed form for any n.
Reports, per scale, the minimal n where the share leaves 0 and where it passes
25%/50%. Model-based extrapolation (normality, sigma fixed at the 3-seed estimate).
Writes seed_budget_curve.json. NFT_R overridable.
"""
import os, json
import numpy as np
import pandas as pd
...
with open(os.path.join(HERE, "seed_budget_curve.json"), "w") as f:
    json.dump(OUT, f, indent=1)
print("wrote seed_budget_curve.json")
