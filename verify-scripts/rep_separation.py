#!/usr/bin/env python3
"""Regenerate grid20m_rep_separation.json from the shipped per-cell eval_final.json
files (rep_evals/): the five replicated 20M cells' run-level differences and the
run-noise-corrected interaction bounds. NFT_STAGE2 overridable."""
import json, os, sys
import numpy as np
...
REPS = {"rep1": ("rep1_i1_o1", 1, 1), "rep2": ("rep2_i2_o2", 2, 2),
        "rep3": ("rep3_i3_o3", 3, 3), "rep4": ("rep4_i4_o4", 4, 4),
        "rep5": ("rep5_i2_o3", 2, 3)}
...
for name, (rd, i, o) in REPS.items():
    orig = json.load(open(os.path.join(EVAL, f"i{i}_o{o}", "eval_final.json")))
...
grid = json.load(open(os.path.join(ST2, "grid20m_analysis.json")))["grid"]
per_task = {}
for t in PRIMARY:
...
with open(os.path.join(HERE, "..", "data", "self_run_controls", "grid20m_rep_separation.json"), "w") as f:
