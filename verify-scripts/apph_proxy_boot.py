#!/usr/bin/env python3
"""apph_proxy_boot: the 20M proxy-calibration
...
Protocol: per run, x_s = relSD over the last-5 checkpoints
...
Input: a compact CSV (grid20m_ckpt_series.csv) with columns run,step,blimp,lambada_openai,
derived from the stage-2 runs' eval_step*.json + eval_final.json. Writes apph_proxy_boot.json.
"""
import os, json
import numpy as np
import pandas as pd
...
CSV = os.environ.get("NFT_STAGE2_SERIES",
                     os.path.join(_ROOT, "data", "self_run_controls", "grid20m_ckpt_series.csv"))
...
json.dump(out, open(os.path.join(HERE, "apph_proxy_boot.json"), "w"), indent=1)
print("wrote apph_proxy_boot.json")
