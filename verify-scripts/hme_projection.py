#!/usr/bin/env python3
"""hme_projection: the HME removes the seed effect g_r estimated from the same
...
Reads dd_tidy.parquet via NFT_DD; HME code via NFT_HME_SRC. Writes hme_projection.json.
"""
import json, os, sys
import numpy as np
import pandas as pd
...
HME_SRC = os.environ.get(
    "NFT_HME_SRC",
    os.path.join(_ROOT, "code", "enhancement3"))
DD = os.environ.get(
    "NFT_DD",
    os.path.join(_ROOT, "data", "analysis", "dd_tidy.parquet"))
...
json.dump(out, open(os.path.join(HERE, "hme_projection.json"), "w"), indent=1)
