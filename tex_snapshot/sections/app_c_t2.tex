...
$\sigma_{\mathrm{init}}/\sigma_{\mathrm{order}}$ with F and bootstrap CIs (all 95\%), for 9 primary-like
and 18 bpb tasks, plus the F interval at the Bonferroni level ($\alpha = 0.05/27$;
conservative here, since 8 task identities appear in both panels). No task's CI fits inside
the $2\times$ equivalence band
($0/27$); the two tasks whose uncorrected F CIs exclude 1 do so in opposite directions, and
...
intervals (df 9 and 8) and run-level bootstrap intervals (2{,}000 resamples). We report band
...
\caption{\textbf{What it would take to decide source equivalence.} Width of the 95\%
variance-ratio CI (a pure function of degrees of freedom), the window of point estimates that
...
\begin{tabular}{@{}cccc@{}}
\toprule
$n$ per arm & 95\% CI span & acceptance window for $\hat r$ & pass prob.\ if true ratio $= 1$ \\
\midrule
...
The width of a variance-ratio CI is a pure function
of degrees of freedom, $F_{0.975}(\nu,\nu)$. With $n = 3$ per arm the interval spans
$39\times$; $n = 6$ spans $7.15\times$; $n = 8$ spans $4.99\times$; even $n = 10$ spans
$4.03\times$, still wider than the $4\times$ band. The S\&N arms (df 9/8, span $4.23\times$)
...
Remark~\ref{prop:t2-nonid} and Corollary~\ref{cor:t2-2727} are stated in the main text
(Section~\ref{sec:t2-power}); we prove them here. In part (i), the explicit binary example has
...
The $0/27$ of Table~\ref{tab:t2} is thus the
finite-sample shadow of a structural fact: no enlargement of the single-source arms can
...
(Section~\ref{sec:t2-point}); what removes the single-source floor as a task-level safe
...
\begin{figure}[!tbp]
\centering
\includegraphics[width=0.44\linewidth,height=0.15\textheight,keepaspectratio]{figures/F3_source_equivalence.pdf}
