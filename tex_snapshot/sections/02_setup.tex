...
We audit the three instruments as they are \emph{used}. Terminology (look-up table:
...
appendix holds ``the shuffle seed fixed across all runs'', i.e.\ init-only as implemented), but
either way the floor is used as a stand-in for the noise of \emph{all} random sources combined;
...
ranking of 25 data recipes at a single 150M proxy scale predicts the better recipe at 1B with
${\sim}80\%$ pairwise accuracy ($80.2\%$ under their single-seed protocol, $82.7\%$ under
3-seed means; Section~\ref{sec:t3}); we audit the residual ${\sim}20\%$.

\paragraph{Assets and declared defects.}
All evidence comes from three public assets released with per-seed resolution:
\textbf{DataDecide} (14 sizes, 4M--1B, $\times$ 25 recipes $\times$ 3 seeds; OLMES suite and
11-domain perplexities; $100$ tokens per parameter, ${\sim}5\times$Chinchilla
...
read band-membership counts as descriptive tallies, not as independent tests. The
...
\paragraph{The audit at a glance.}
Table~\ref{tab:audit-map} collects the three audits' outcomes (terminology:
Table~\ref{tab:terms}, Appendix~\ref{app:setup}).

\begin{table}[!tbp]
\centering
\caption{\textbf{The three audits at a glance.} Each instrument reproduces in its home
setting; the failures differ in kind, and so do the licenses (Sections~\ref{sec:t1}--\ref{sec:t3}).}
\label{tab:audit-map}
\scriptsize
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{0.85}
\begin{tabular}{@{}p{0.15\linewidth}p{0.17\linewidth}p{0.24\linewidth}p{0.36\linewidth}@{}}
\toprule
instrument & home replication & transfer failure & verdict / usage license \\
\midrule
checkpoint-noise proxy (S\&N) & $R^2 = 0.81$--$0.99$ & under-reads seed noise
${\sim}1.6\times$ on accuracy (more at small scale), over-reads $1.1$--$7\times$ on perplexity;
ranks recipes' noise at chance & magnitude estimate only with dense grid $+$ detrended
continuous metric $+$ seed-semantics conversion; else measure seeds \\
single-source noise floor & n/a (practice audit) & all-source floor not identifiable from
single-source arms, at any arm size & structural: needs a bundled or crossed design, not more
single-source runs \\
recipe ranking (DataDecide) & $80.2\%$ pairwise at 150M, to the digit & SNR $\le 1.3$ to 60M;
4M macro ranking inverts against 1B ($r = -0.56$) & pairwise-accurate at the recommended scale but argmax-fragile (the 150M argmax pick
ranks 21st of 25 at 1B); where the decidable share is $0$ (on this panel: every scale
$\le 60$M), abstain and measure seeds \\
\bottomrule
\end{tabular}
\end{table}
