...
We audit the three instruments as they are \emph{used}. Terminology: a \emph{decision
...
configuration (model scale, evaluation suite, checkpoint grid, metric) in which its advertised
performance was originally established; and its \emph{usage license} is the set of conditions
under which its readings remain trustworthy (stated per instrument in
Sections~\ref{sec:t1}--\ref{sec:t3}). \textbf{The checkpoint-noise proxy}: Signal \&
...
(a bundled reading shifts Section~\ref{sec:t2}'s question, not its arithmetic). \textbf{Small-scale
...
${\sim}80\%$ pairwise accuracy; we audit the residual ${\sim}20\%$.
...
Section~\ref{sec:t3} is the \emph{decision-instrument SNR}: signal $=$ the SD of the 25 recipe
...
Appendix~\ref{app:t3}, Table~\ref{tab:snr-ci}).
