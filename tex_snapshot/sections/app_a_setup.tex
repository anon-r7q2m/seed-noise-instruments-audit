...
1B runs on one recipe: 10 init-only + 9 data-order-only + 1 densely evaluated; 19 tasks (27
primary$+$bpb readouts) &
20 runs &
...
seeds --- the training pipeline derives both streams from the run's single seed argument (its
\texttt{create\_ladder\_over\_scale\_script.py} passes one \texttt{--seeds} value per run into
the OLMo trainer's single \texttt{seed} config field) --- so it yields all-source noise only;
source decomposition is possible solely on the S\&N
