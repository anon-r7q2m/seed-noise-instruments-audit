...
cross-task Pearson correlation, in raw and log space. One honesty note on the headline
...
shows the home claim is robust to every one of them (\texttt{r28\_sn\_exact\_protocol.json}).
...
data-order arms. The exact-protocol row reproduces the published $0.82$/$0.86$; no single
...
tasks per cell (350 cells; Figure~\ref{fig:t1-panels}b). The pooled per-task grid is
...
perfect proxy under the same sampling noise ($\texttt{r5\_null\_band.json}$); the
family$\times$task$\times$seed bootstrap SE of the pooled median is $0.03$--$0.06$
($\texttt{r9\_joint\_bootstrap.json}$).
...
identifiability at $n = 3$, not on the proxy: better instruments could exist; under the conventional certificate at this seed budget, none can be \emph{validated} against a 3-seed truth beyond this resolution with useful power. (ii) On the macro
