...
reaches only ${\approx}0.53$ (dots) --- Appendix~\ref{app:t1-ranking}. (b) On the accuracy
readout the proxy under-reads seed noise at every non-truncated scale (median truth-SD /
proxy-SD; parity
and $\sqrt{2}$ references; 750M is budget-truncated, and its raw ratio is $0.95$) --- Appendix~\ref{app:t1-port}. (c) On
perplexity the raw proxy \emph{over}-reads $1.1$--$7\times$ (vermillion); detrending
returns the two largest scales to parity --- Appendix~\ref{app:t1-ppl}.}
\label{fig:t1-panels}
\end{figure}
...
cross-task Pearson correlation, in raw and log space. One caveat on the headline
$R^2$: the raw-space $0.99$ rides on \texttt{copycolors}, the one task whose noise exceeds the
others' by $7\times$ --- dropping it gives $R^2 = 0.40$ (init, raw) and $0.26$ (log); the
data-order arm holds up better ($0.69$/$0.73$). The correlation is real but its
informativeness is concentrated in the high-noise task.

\paragraph{Protocol alignment with the original Figure 7.} Their figure's protocol differs
from ours in four components; re-implementing each exactly (same released parquet throughout)
shows the home claim is robust to every one of them (\texttt{sn\_exact\_protocol.json}).
Their printed $R^2$ values agree with the log-space recomputation (the figure's axes are linear).

\begin{table}[!htbp]
\centering
\caption{\textbf{Protocol alignment for the home replication.} Per row, one component moved
between Signal \& Noise's Figure 7 protocol and ours; log-space $R^2$ for the init-seed and
data-order arms. The exact-protocol row reads $0.82$/$0.84$ against the published $0.82$/$0.86$; no single
deviation --- nor our combination --- changes the qualitative result.}
\label{tab:t1-align}
\footnotesize
\begin{tabular}{@{}lcccccc@{}}
\toprule
variant & tasks & proxy window & target smoothing & step truncation & init $R^2$ (log) & order $R^2$ (log) \\
\midrule
S\&N Fig.\ 7 (published) & 8 & last 20 & last 20 & --- & 0.82 & 0.86 \\
exact re-implementation & 8 & last 20 & last 20 & --- & 0.82 & 0.84 \\
\quad + ninth task (\texttt{copycolors}) & 9 & last 20 & last 20 & --- & 0.93 & 0.90 \\
\quad + last-30 window & 8 & last 30 & last 20 & --- & 0.87 & 0.76 \\
\quad + last-3 target & 8 & last 20 & last 3 & --- & 0.84 & 0.85 \\
\quad + common-step truncation & 8 & last 20 & last 20 & step $69000$ & 0.60 & 0.84 \\
this paper (Table~\ref{tab:t1a}) & 9 & last 30 & last 3 & common step $69000$ & 0.81 & 0.88 \\
\bottomrule
\end{tabular}
\end{table}
...
tasks per cell (350 recipe--scale cells; Figure~\ref{fig:t1-panels}b). The pooled per-task grid is
$3{,}497 = 3{,}500$ cells minus three degenerate ones (zero seed variance at 1B
\texttt{arc\_challenge} on DCLM-Baseline 50\%/Dolma 50\%; zero checkpoint spread at 4M
\texttt{boolq}/\texttt{piqa} on two Dolma variants).
...
ceiling --- computed under independent per-seed errors --- is biased downward,
...
median-basis log-space correction.
 Across sizes, $60$--$96\%$ of cells are compatible with a
...
\paragraph{Pooled summary.} Across all $3{,}497$ cells: median $y/x$ $1.47$ raw ($1.61$ with the
median-basis correction); $53.9\%$ of cells inside the $2\times$ band versus $67.9\%$ for a
perfect proxy under the same sampling noise ($\texttt{null\_band.json}$); the
family$\times$task$\times$seed bootstrap SE of the pooled median is $0.03$--$0.06$
($\texttt{joint\_bootstrap.json}$).
...
contains sampling noise. A perfect proxy would reach median $\tau \approx 0.53$
($0.49$--$0.56$ across scales); the checkpoint
proxy reaches $0.03$. (Script: \texttt{verify-scripts/perfect\_proxy\_ceiling.py}.)
...
the perfect-proxy ceiling (median over cells of the Monte-Carlo median). The macro variant ranks
...
Two qualifications. (i) The ceiling is a \emph{perfect-proxy reference} at $n = 3$, not on the proxy: better instruments could exist; under the conventional certificate at this seed budget, none can be \emph{validated} against a 3-seed truth beyond this resolution with useful power. (ii) On the macro
...
\label{prop:t1-bound}
Assume i.i.d.\ normal arms, i.i.d.\ tasks, a common cross-task dispersion $\sigma_a^2$ of the latent
...
with $b(\nu) := \mathrm{E}[\varepsilon] = \mathrm{E}[\log S - \log \sigma]$ at $\nu$ df, $b(2) = -0.289$ nat vs.\ $\log c_4(3) = -0.121$). This is an errors-in-variables model with
...
power), so within this limit experiment the bound is a lower bound on the requirement.
\end{proof}
...
even in the original setting. For $99.7\%$ of our 350 recipe--scale cells the perfect proxy's \emph{population}
...
the verifier. (Script: \texttt{verify-scripts/prop1\_mc\_calibration.py} in the released
...
its ceiling is biased downward here). Under a leave-one-seed-out variant (the proxy computed on
...
$\hat\lambda$ as consistent with the advertised level, not as evidence for it; certification at useful power remains out of reach either way (Corollary~\ref{cor:t1-n3}).

\begin{table}[!htbp]
\centering
\caption{\textbf{The leave-one-seed-out (LOO) porting.} Per size: median cross-task $R_{\log}$ with
...
window) repairs the calibration --- the license criterion: the detrended median ratio within $10\%$ of $1$ \emph{and} a majority of cells inside the $2\times$ band --- only at the two largest scales, at median ratio
