...
150M & 52 & 52 [25, 101] & 13\% [8, 57]  & 7.2\% [6.3, 12.7] & 10.9\% [1.9, 30.2] & 0.047 & 0.036 [0.002, 0.044] \\
...
fabric crosses throughout ($r=-0.56$). Right (bpb readout): the same recipes hold their
ranks ($r=+0.87$).}
\label{fig:alluvial}
\end{figure}
...
Dropping the seven tasks whose 4M pairwise accuracy is indistinguishable from chance
(recipe-cluster bootstrap interval covers $50\%$) leaves \texttt{arc\_easy}, \texttt{csqa},
and \texttt{piqa}; on that subset the 4M-macro vs 1B correlation reads $+0.56$
[$0.14$, $0.70$]. Dropping only the three tasks whose best 4M recipe is within Welch noise of
the task's chance rate (\texttt{arc\_challenge}, \texttt{mmlu}, \texttt{openbookqa}) leaves
$-0.33$ [$-0.54$, $0.16$]. The inversion is thus a property of the full macro readout's task
...
\begin{table}[!htbp]
\centering
\caption{\textbf{Decidable share of the 300 macro pairs under three multiplicity rules.}
BH (main text), Benjamini--Yekutieli, and a parametric max-$T$ bootstrap.}
\label{tab:decidable}
\footnotesize
\begin{tabular}{@{}lccc@{}}
\toprule
