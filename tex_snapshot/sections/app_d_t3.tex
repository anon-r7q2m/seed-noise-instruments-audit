...
grid (median grid density 23 at 60M vs 13 at 20M). On the bpb readout the SNR reads
...
zero-variance probability per recipe). The zero share at ${\le}60$M is invariant to the rule.
...
${\le}20$M share at zero under BH except $7\%$ at 20M, but reads $34\%$/$19\%$ at 60M under
...
two variance models.} Left: pair-specific Welch variances under BH (main text),
...
Table~\ref{tab:snr-ci}), and shown as a sensitivity bound: the $\le$20M zero is invariant to
the variance model, while the 60M share is not ($0\%$ pair-specific; $34\%$/$19\%$ pooled
under BH/BY).}
\label{tab:decidable}
\footnotesize
