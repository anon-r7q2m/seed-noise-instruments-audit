...
(`check_numbers.py`, 53 checks), the Table 1 reference implementation reproduced digit for
digit (`rule3_repro.py`), and the sensitivity analyses (finite-sample
...
- `verify-scripts/` — the number gates and the sensitivity analyses (entry points);
  `reproduce.sh` step 0 regenerates every intermediate JSON from the
...
- `data/self_run_controls/` — the self-run controls' analysis outputs (the 20M crossed grid and
